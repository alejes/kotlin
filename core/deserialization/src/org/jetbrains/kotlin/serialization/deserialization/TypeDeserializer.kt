...
class TypeDeserializer(
    ...
    fun simpleType(proto: ProtoBuf.Type, additionalAnnotations: Annotations = Annotations.EMPTY): SimpleType {
        val localClassifierType = when {
            proto.hasClassName() -> computeDefaultTypeForLocalClassifier(proto.className)
            proto.hasTypeAliasName() -> computeDefaultTypeForLocalClassifier(proto.typeAliasName)
            else -> null
        }
    ...
    private fun computeDefaultTypeForLocalClassifier(className: Int): SimpleType? {
        if (c.nameResolver.getClassId(className).isLocal) {
            return c.components.localClassifierTypeSettings.defaultStubTypeForLocalClassifiers
        }
        return null
    }
